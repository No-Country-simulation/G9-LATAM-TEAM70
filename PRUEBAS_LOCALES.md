...
- Un servicio de Machine Learning accesible, si se usarán los endpoints de clasificación.

La API no contiene scripts de Python ni archivos `.pkl`. El modelo se consume desde un servicio HTTP externo configurado mediante `ML_SERVICE_URL`.
...
Si `ML_SERVICE_URL` no se define, la API intenta conectarse a `http://localhost:5000`.
...
Los endpoints de contenido requieren un JWT. Obtén uno mediante `POST /api/auth/login` y envíalo en el encabezado:
